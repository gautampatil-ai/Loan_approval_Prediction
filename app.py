...
def load_pipeline():
    ...
    if not os.path.exists(model_path):
        st.error(
            f"⚠️ Model file not found at `{model_path}`. Please run `train.py` first."
        )
        st.stop()
...
# Header Section
st.markdown(
    '<div class="main-header">🏦 Enterprise Loan Risk Engine</div>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="sub-header">Evaluate applicant default probability and approval confidence in real-time.</div>',
    unsafe_allow_html=True,
)
...
with st.sidebar.form(key="applicant_form"):
    ...
    education = st.selectbox(
        "Education Level", options=["Graduate", "Not Graduate"]
    )
    ...
    income_annum = st.number_input(
        "Annual Income ($)", min_value=0, value=7500000, step=50000
    )
    loan_amount = st.number_input(
        "Requested Loan Amount ($)", min_value=0, value=15000000, step=50000
    )
    loan_term = st.number_input(
        "Loan Term (Years)", min_value=1, max_value=30, value=12, step=1
    )
    cibil_score = st.slider(
        "CIBIL Credit Score", min_value=300, max_value=900, value=750, step=1
    )

    st.subheader("Valued Assets ($)")
    residential_assets_value = st.number_input(
        "Residential Asset Value", min_value=0, value=4000000, step=25000
    )
    commercial_assets_value = st.number_input(
        "Commercial Asset Value", min_value=0, value=2500000, step=25000
    )
    luxury_assets_value = st.number_input(
        "Luxury Asset Value", min_value=0, value=8000000, step=25000
    )
    bank_asset_value = st.number_input(
        "Bank Asset Value", min_value=0, value=5000000, step=25000
    )
...
with col_left:
    ...
    summary_df = pd.DataFrame({
        ...
        "Value": [
            no_of_dependents,
            education,
            "Yes" if self_employed == "Yes" else "No",
            f"${income_annum:,}",
            f"${loan_amount:,}",
            f"{loan_term} Years",
            cibil_score,
...
with col_right:
    ...
    if submit_button:
        try:
            # ENCODING FIX: Convert categorical text inputs to numerical values (1/0)
            ...
            input_data = pd.DataFrame([{
                ...
                'education': education_num,  # Passed as int (1 or 0)
                'self_employed': self_employed_num,  # Passed as int (1 or 0)
            ...
            if has_proba:
                ...
                if 1 in classes:
                    approved_idx = list(classes).index(1)
                    rejected_idx = list(classes).index(0)
                    prob_approved = probabilities[approved_idx]
                    prob_rejected = probabilities[rejected_idx]
                else:
            ...
            if prediction == 1 or str(prediction).lower() in [
                ...
                st.info(
                    "Applicant meets credit risk standards and asset valuation thresholds."
                )
            else:
                st.error("❌ **LOAN APPLICATION REJECTED**")
                if has_proba:
                    st.metric(
                        label="Rejection Risk Probability",
                        value=f"{prob_rejected * 100:.1f}%",
                    )
                    st.progress(float(prob_rejected))
                st.warning(
                    "Applicant does not meet minimum risk criteria or asset ratio bounds."
                )
        ...
        st.info(
            "👈 Adjust applicant parameters in the sidebar and click **Evaluate Loan Eligibility**."
        )
